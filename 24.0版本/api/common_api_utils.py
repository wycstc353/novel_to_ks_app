...
def _get_proxies(proxy_config):
    ...
    Args:
        proxy_config (dict or None): 包含代理设置的字典。
            Google/通用: {"use_proxy": bool, "proxy_address": str, "proxy_port": str}
    ...
    """
    proxies = None
    use_key = "use_proxy" # 默认通用/Google API 的 key
    addr_key = "proxy_address"
    port_key = "proxy_port"
    api_name = "通用/Google" # 默认 API 名称
    ...
    if proxy_config and "nai_use_proxy" in proxy_config:
        ...
        api_name = "NAI"
        logger.debug(f"检测到 NAI 特定代理配置键。") # 记录检测到 NAI 配置
    ...
    if proxy_config and proxy_config.get(use_key) and proxy_config.get(addr_key) and proxy_config.get(port_key):
        ...
        if addr and port:
            ...
            if not addr.startswith(('http://', 'https://', 'socks5://', 'socks4://')):
                 ...
                 logger.debug(f"代理地址 '{addr}' 未指定协议，默认使用 http。") # 记录默认协议
            else:
                 url = f"{addr}:{port}" # 如果已有协议头，直接拼接端口

            proxies = {"http": url, "https": url} # 同时为 http 和 https 设置代理
            logger.info(f"[{api_name} API Helper] 使用代理: {url}") # 记录使用的代理
        else:
            logger.warning(f"[{api_name} API Helper] 代理已启用但地址或端口为空，将不使用代理。") # 记录配置不完整警告
    else:
        logger.info(f"[{api_name} API Helper] 未配置或未启用代理。") # 记录未使用代理
    return proxies