    ...
    try:
        ...
        if config_type == "llm_global":
            ...
            for key in ['saveDebugInputs', 'enableStreaming', 'use_proxy']:
                if key in final_config: # 检查 key 是否存在
                    final_config[key] = str(final_config.get(key)).lower() == 'true'
                else: # 如果 key 不存在，使用默认值
                    final_config[key] = defaults.get(key, False)
        ...
        # 新增：GPT-SoVITS 配置校验
            ...
            # Ref Free (bool)
            if 'ref_free' in final_config:
                final_config['ref_free'] = str(final_config.get('ref_free')).lower() == 'true'
            else:
                final_config['ref_free'] = defaults.get('ref_free', False)
...
# --- 修改：加载 Character Profiles (添加旧格式迁移) ---
        ...
        try:
            ...
            for name_key, data in profiles.items():
                if isinstance(data, dict):
                    # 检查是否缺少新结构的关键字段 'display_name'
                    # 如果缺少 'display_name'，则认为是旧格式
                    if "display_name" not in data:
                        migration_needed = True
                        print(f"  检测到旧格式 Profile: '{name_key}'，进行迁移...")
                        migrated_profiles[name_key] = {
                            "display_name": name_key, # 使用 key 作为 display_name
                            "replacement_name": "",   # 替换名称默认为空
                            "positive": data.get("positive", ""), # 保留原有提示词
                            "negative": data.get("negative", "")
                        }
                    else:
                        # 如果是新格式或已经迁移过的格式，直接复制
                        # 同时确保 replacement_name 存在，如果不存在则添加空字符串
                        if "replacement_name" not in data:
                            data["replacement_name"] = ""
                        migrated_profiles[name_key] = data
                else:
                    # 如果值不是字典，忽略此条目
                    print(f"警告：加载人物设定时，Key '{name_key}' 的值不是字典，已忽略。")

            if migration_needed:
                print("人物设定文件格式已从旧版本迁移到新版本。")
                # 可以选择性地通知用户
                messagebox.showinfo("格式迁移", "加载的人物设定文件是旧格式，已自动转换为新格式。\n请检查“替换名称”字段。", parent=parent_window)

            print(f"人物设定已从 '{filepath}' 加载并处理。")
            return migrated_profiles, filepath # 返回迁移后的字典和文件路径
