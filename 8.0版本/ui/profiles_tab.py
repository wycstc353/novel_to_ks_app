...
class ProfilesTab(ctk.CTkFrame):
    ...
    def __init__(self, master, config_manager, app_instance):
        ...
        # 新的数据结构: key 是原始/显示名称, value 是包含 display_name, replacement_name, positive, negative 的字典
        # { "原始名称": {"display_name": "原始名称", "replacement_name": "替换名", "positive": "...", "negative": "..."} }
    ...
    def render_profiles_list(self):
        ...
        # --- 获取并排序人物名称 (按 display_name 排序可能更好，但需要额外处理，暂时按 key 排序) ---
        ...
        if not names:
            ...
            no_profiles_label.grid(row=0, column=0, columnspan=5, padx=10, pady=20, sticky="n") # 跨越所有列
            # 禁用保存按钮，因为没有内容可保存
            ...
            # --- 遍历人物 ---
            for name_key in names: # 使用 key 遍历
                profile_data = self.character_profiles.get(name_key, {})

                # 再次校验获取到的数据是否为字典
                if not isinstance(profile_data, dict):
                     print(f"警告: 人物 '{name_key}' 的数据不是字典格式，已跳过渲染。数据: {profile_data}")
                     continue # 跳过这个无效的人物条目

                # --- 为每个人物创建一个独立的 Frame ---
                entry_frame = ctk.CTkFrame(self.scrollable_frame, fg_color="transparent")
                entry_frame.grid(row=current_row, column=0, columnspan=5, pady=(0, 8), sticky="ew") # 跨越所有列
                # 配置 entry_frame 内部列权重
                entry_frame.grid_columnconfigure(1, weight=1) # 替换名称列
                entry_frame.grid_columnconfigure(2, weight=1) # 正向提示列
                entry_frame.grid_columnconfigure(3, weight=1) # 负向提示列

                # --- 定义回调函数 (使用闭包捕获当前循环的 key) ---
                # 创建删除按钮的回调，捕获当前的 key
                def create_remove_callback(key): # 使用 key
                    return lambda: self.remove_profile(key)
                # 创建更新字段的回调，捕获当前 key、字段类型和对应的 StringVar
                def create_update_callback(key, field_type, entry_var): # 使用 key
                    # 当输入框失去焦点 (<FocusOut>) 时触发更新
                    return lambda event=None: self.update_profile_field(key, field_type, entry_var.get())

                # --- 创建 UI 元素 ---
                # 显示名称 (标签，不可编辑)
                display_name = profile_data.get("display_name", name_key) # 获取显示名称，默认为 key
                name_label = ctk.CTkLabel(entry_frame, text=display_name, anchor="w", width=120) # 固定宽度或根据需要调整
                ...
                repl_entry.bind("<FocusOut>", create_update_callback(name_key, "replacement_name", repl_var)) # 绑定更新
                ...
                remove_btn = ctk.CTkButton(entry_frame, text="删除", width=60,
                                          ...
                                          command=create_remove_callback(name_key)) # 绑定删除回调
    ...
    # handle_name_change 不再需要，因为显示名称不可编辑

    def update_profile_field(self, name_key, field_type, value):
        """更新内存中指定人物(key)的指定字段 (在输入框失去焦点时触发)"""
        print(f"[ProfilesTab] 更新字段请求：Key='{name_key}', 字段='{field_type}', 新值='{value[:30]}...'") # 只打印部分值避免过长日志

        # --- 直接修改内存中的字典 ---
        # 检查人物 Key 是否存在于字典中
        if name_key in self.character_profiles:
            # 允许更新 "replacement_name", "positive", "negative"
            if field_type in ["replacement_name", "positive", "negative"]:
                ...
                if isinstance(self.character_profiles[name_key], dict):
                    ...
                    print(f"[ProfilesTab] 内存中 '{name_key}' 的 '{field_type}' 字段已更新。")
                     ...
                     # 如果值不是字典，记录错误
                     print(f"错误：人物 '{name_key}' 在内存中的设定数据不是字典格式！无法更新字段 '{field_type}'。")
            else:
                 # 如果字段类型未知，记录警告
                 print(f"警告: 尝试更新未知的字段类型 '{field_type}' 对于人物 '{name_key}'")
        else:
            # 如果人物 Key 在字典中找不到
            print(f"警告: 尝试更新不存在的人物 Key '{name_key}' 的字段 '{field_type}'。")
        # 注意：这里只更新内存数据，不重新渲染整个列表，因为只是字段值变化，
        # UI 上的输入框已经通过 StringVar 显示了新值。

    def remove_profile(self, name_key_to_remove):
        """从内存和 UI 中移除指定的人物设定 (通过 key)"""
        ...
        if name_key_to_remove in self.character_profiles:
            ...
            # 弹出确认对话框
            if messagebox.askyesno("确认删除", f"确定要删除人物 '{display_name}' (Key: {name_key_to_remove}) 吗？\n此操作将立即从当前编辑的设定中移除，但不会影响已保存的文件。", parent=self):
                # --- 关键：直接从内存字典中删除 ---
                try:
                    ...
                    # 理论上不应该发生，因为前面检查过存在性
                    print(f"错误：尝试删除人物 Key '{name_key_to_remove}' 时发生 KeyError (可能并发问题?)")
                    self.render_profiles_list() # 尝试刷新 UI
                except Exception as e:
                    print(f"错误：删除人物 Key '{name_key_to_remove}' 时发生意外错误: {e}")
                    traceback.print_exc()
                    messagebox.showerror("删除错误", f"删除人物时出错:\n{e}", parent=self)
                    self.render_profiles_list() # 尝试刷新 UI
        else:
            # 如果尝试删除一个不存在的人物 Key (可能UI状态不同步)
            print(f"警告: 尝试删除不存在的人物 Key '{name_key_to_remove}'。")
    ...
    def add_new_profile(self):
        ...
        if new_name_raw is not None: # 检查用户是否点击了 "OK" (即使输入为空)
            ...
            if new_name in self.character_profiles: # 检查 Key 是否已存在
                messagebox.showerror("名称冲突", f"人物名称 (Key) '{new_name}' 已经存在！", parent=self)
                print(f"错误：用户尝试添加已存在的 Key '{new_name}'。")
                return # 不进行添加
    ...
    def load_profiles_from_file(self):
        """加载人物设定文件，更新内存字典和 UI (包含旧格式迁移)"""
        print("[ProfilesTab] 请求加载人物设定文件...")
        # 调用 config_manager 的加载函数 (它现在包含迁移逻辑)
        ...
        if result:
            ...
            print(f"[ProfilesTab] 加载并可能迁移后的人物设定数据: {profiles}") # 打印加载的数据
    ...
    def save_profiles_to_file(self):
        ...
        # --- 在保存前最后一次检查 Key 是否有效 ---
        # 检查是否存在空 Key 或只包含空格的 Key
        invalid_keys = [key for key in self.character_profiles.keys() if not key or key.isspace()]
        if invalid_keys:
             messagebox.showerror("内部错误", f"发现无效的人物 Key（为空或只包含空格）：\n{', '.join(invalid_keys)}\n\n请删除并重新添加。", parent=self)
             print(f"[ProfilesTab] 保存取消：发现无效 Key: {invalid_keys}")
             # 可以考虑调用 render_profiles_list() 来刷新 UI
    ...
    def get_profiles_for_step2(self):
        """
        返回供步骤二使用的 profiles 字典 (包含替换逻辑) 和 JSON 字符串。
        返回字典: { "原始名称Key": {"display_name": ..., "replacement_name": ..., ...} }
        返回 JSON: { "有效名称": {"positive": ..., "negative": ...} }
        """
        ...
        if invalid_keys:
             messagebox.showerror("名称错误", f"执行操作前发现无效的人物 Key（为空或只包含空格）：\n{', '.join(invalid_keys)}\n\n请在“人物设定”标签页中修正后再继续。", parent=self)
             print("[ProfilesTab] 获取数据失败：发现无效 Key。")
             self.render_profiles_list() # 刷新列表以显示当前状态
             return None, None # 返回两个 None

        # --- 构建步骤二所需的 JSON (使用有效名称作为 Key) ---
        effective_profiles_for_json = {}
        for key, data in current_profiles.items():
            if isinstance(data, dict):
                # 确定有效名称：优先使用替换名称，否则使用显示名称，最后使用 Key
                ...
                # 如果数据格式不正确，记录警告并跳过
                print(f"警告：人物 Key '{key}' 的数据格式无效，已在生成 JSON 时跳过。")
        ...
        try:
            ...
            # 返回原始的、包含完整结构的字典 和 生成的、只包含有效名称和提示词的 JSON
            return current_profiles.copy(), json_string
        except Exception as e:
    ...
    # get_profiles_json 方法被 get_profiles_for_step2 取代
    # def get_profiles_json(self):
    #     _, json_string = self.get_profiles_for_step2()
    #     return json_string