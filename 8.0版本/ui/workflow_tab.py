...
class WorkflowTab(ctk.CTkFrame):
    ...
    def build_ui(self):
        ...
        # --- 新增：导入名称按钮 ---
        self.import_names_button = ctk.CTkButton(step1_controls, text="导入名称到人物设定", command=self.import_names_from_step1, state="disabled") # 初始禁用
        self.import_names_button.pack(side="left", padx=(0, 10))
        # --- 导入按钮结束 ---
    ...
    def toggle_specific_images_entry(self):
        ...
        if self.img_gen_scope_var.get() == "specific":
            ...
            self.specific_images_var.set("") # 清空内容
    ...
    def toggle_specific_speakers_entry(self):
        ...
        if self.audio_gen_scope_var.get() == "specific":
            ...
            self.specific_speakers_var.set("") # 清空内容
            ...
            if text is not None:
                if isinstance(element, (ctk.CTkLabel, ctk.CTkButton)):
                    ...
                    try:
                        ...
                        traceback.print_exc()
                    # 标记文本已处理，避免后续 configure 再次尝试设置 text
                    text = None
            ...
            if task_id == "步骤三 (BGM+KAG)":
                ...
                if use_final_stream:
                    ...
                    for status, data in api_helpers.stream_google_response(
                        ...
                        kag_prompt, 0.1, llm_config.get('maxOutputTokens'), # KAG 转换强制低 temperature
    ...
    def run_step2_enhance(self):
        ...
        llm_config = self.app.get_llm_config()
        # --- 修改：获取完整的 Profiles 字典和用于 Prompt 的 JSON ---
        # 注意：这里假设 ProfilesTab 实例存在且可用
        ...
        if not llm_config or not all(k in llm_config for k in ['apiKey', 'apiEndpoint', 'modelName']): messagebox.showerror("配置错误", "请先在 'LLM 与全局设置' 标签页中配置 LLM！", parent=self); return
        # --- 修改：检查 profiles_dict 和 profiles_json_for_prompt ---
        if profiles_dict is None or profiles_json_for_prompt is None:
            # get_profiles_for_step2 内部会处理 None 和弹出警告/错误
            print("步骤二取消：从 ProfilesTab 获取数据失败。")
            return
        # --- 修改结束 ---
        ...
        proxy_config = {k: llm_config.get(k) for k in ["use_proxy", "proxy_address", "proxy_port"]}
        use_stream = llm_config.get("enableStreaming", True)
        # --- 修改：使用 profiles_json_for_prompt ---
        # 注意：formatted_text 仍然是步骤一的原始输出，名称替换将在 task_llm_enhance 内部进行
        prompt = self.app.prompt_templates.PROMPT_ENHANCEMENT_TEMPLATE.format(
            pre_instruction=llm_config.get('preInstruction',''),
            post_instruction=llm_config.get('postInstruction',''),
            character_profiles_json=profiles_json_for_prompt, # 使用生成的有效 JSON
            formatted_text_chunk=formatted_text # 原始格式化文本（未替换名称）
        )
        # --- 修改结束 ---
        ...
        if use_stream:
            ...
            # --- 修改：传递 profiles_dict ---
            args = (self.api_helpers, self.app.prompt_templates, llm_config, formatted_text, profiles_dict) # 传递完整字典
            # --- 修改结束 ---
            is_stream = False
    ...
    def import_names_from_step1(self):
        """从步骤一结果中提取 [名字] 标记，并添加到人物设定标签页"""
        print("请求从步骤一结果导入名称...")
        formatted_text = self.structured_text_widget.get("1.0", "end-1c")
        if not formatted_text.strip():
            messagebox.showwarning("无内容", "步骤一结果为空，无法导入名称。", parent=self)
            return

        try:
            # 使用正则表达式查找所有 `[xxx]` 形式的标记
            # 改进正则，只匹配那些看起来像名字的标记（不含冒号，非 image/name 关键字）
            # 匹配以 '[' 开头，']' 结尾，中间不包含 ']' 或 ':' 的内容
            potential_names = re.findall(r'\[([^\]:]+)\]', formatted_text)

            # 过滤掉非名字的标记 (例如 [NAI:...] [image...]) 和空字符串
            # 进一步过滤，只保留那些可能真的是名字的行（例如，只包含 [名字] 的行）
            # 或者简单过滤掉已知非名字的模式
            filtered_names = set()
            for name in potential_names:
                name_lower = name.lower()
                # 排除已知非名字模式和纯数字/特殊字符
                if name_lower not in ['image', 'name'] and not name_lower.startswith('insert_image_here') and name.strip() and not re.fullmatch(r'[\d\W_]+', name):
                     filtered_names.add(name.strip()) # 添加到集合去重并去除首尾空格

            if not filtered_names:
                messagebox.showinfo("未找到名称", "在步骤一结果中未能自动识别出新的人物名称标记。", parent=self)
                return

            # 获取 ProfilesTab 实例
            if hasattr(self.app, 'profiles_tab') and self.app.profiles_tab.winfo_exists():
                profiles_tab = self.app.profiles_tab
                added_count = 0
                skipped_count = 0

                # 遍历找到的唯一名称
                for name in filtered_names:
                    # 检查名称是否已存在于 ProfilesTab 的字典中 (检查 Key)
                    if name not in profiles_tab.character_profiles:
                        # 如果不存在，添加新条目 (使用新数据结构)
                        profiles_tab.character_profiles[name] = {
                            "display_name": name,
                            "replacement_name": "",
                            "positive": "",
                            "negative": ""
                        ...
                        added_count += 1
                        print(f"  > 已添加新名称到人物设定: {name}")
                    else:
                        skipped_count += 1
                        print(f"  > 跳过已存在名称: {name}")

                # 如果有新名称添加，则重新渲染 ProfilesTab 的列表
                if added_count > 0:
                    profiles_tab.render_profiles_list()
                    # 尝试滚动到底部
                    profiles_tab.after(100, profiles_tab._scroll_to_bottom)
                    ...
                    messagebox.showinfo("无需导入", "所有在步骤一结果中识别出的名称均已存在于人物设定中。", parent=self)

            else:
                messagebox.showerror("内部错误", "无法访问人物设定标签页。", parent=self)

        except Exception as e:
            print(f"从步骤一导入名称时出错: {e}")
            traceback.print_exc()
            messagebox.showerror("导入错误", f"从步骤一结果导入名称时发生错误:\n{e}", parent=self)
    # --- 导入名称结束 ---
    ...
    def update_button_states(self, event=None):
        ...
        required_widget_names = [
            'preprocess_button', 'import_names_button', 'enhance_button', 'convert_button', # 添加 import_names_button
        ...
        # --- 修改：导入名称按钮状态 ---
        # 依赖步骤一的结果框（structured_text_widget）是否有内容
